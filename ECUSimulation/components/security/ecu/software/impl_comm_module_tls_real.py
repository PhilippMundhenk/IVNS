...
class TLSRecordLayer(object):
    ...
    def __init__(self, sim_env, transport_layer, ecu_id, monitor_lst=[], jitter=1):
        ...
        self.read_iv = {}  # Initialization Vector for symmetric encryption when receiving
        
        self.write_key = {}  # symmetric key used to encrypt messages
        self.write_mac_key = {}  # key for the MAC when sending
        self.write_iv = {}  # Initialization Vector for symmetric encryption when sending
    ...
    def receive(self, msg_id, message_cipher, sender_id):
        ...
        stream_id = message_cipher.stream_id        
    ...
    def set_keys_from_sec_par(self, w_pars, r_pars, sender_id, stream_id):
        ...
        if self.mode[sender_id][stream_id] == TLSConnectionEnd.SERVER:        
            # write keys    
            G().add_to_three_dict(self.write_key, sender_id, stream_id, sym_get_key(w_pars.bulk_cipher_algorithm, EnumTrafor().to_enum(w_pars.enc_key_length), w_pars.bulk_cipher_algorithm_option, ser_write_ky))
            G().add_to_three_dict(self.write_mac_key, sender_id, stream_id, MACKey(w_pars.mac_algorithm, w_pars.mac_key_length, ser_write_mac_ky))
            G().add_to_three_dict(self.write_iv, sender_id, stream_id, ser_iv)
             
            # read keys
            G().add_to_three_dict(self.read_key, sender_id, stream_id, sym_get_key(r_pars.bulk_cipher_algorithm, EnumTrafor().to_enum(r_pars.enc_key_length), r_pars.bulk_cipher_algorithm_option, cl_write_ky))
            G().add_to_three_dict(self.read_mac_key, sender_id, stream_id, MACKey(r_pars.mac_algorithm, r_pars.mac_key_length, cl_write_mac_ky))
            G().add_to_three_dict(self.read_iv, sender_id, stream_id, cl_iv)

        if self.mode[sender_id][stream_id] == TLSConnectionEnd.CLIENT:
            # write keys   
            G().add_to_three_dict(self.write_key, sender_id, stream_id, sym_get_key(w_pars.bulk_cipher_algorithm, EnumTrafor().to_enum(w_pars.enc_key_length), w_pars.bulk_cipher_algorithm_option, cl_write_ky))
            G().add_to_three_dict(self.write_mac_key, sender_id, stream_id, MACKey(w_pars.mac_algorithm, w_pars.mac_key_length, cl_write_mac_ky))
            G().add_to_three_dict(self.write_iv, sender_id, stream_id, cl_iv)
            
            # read keys
            G().add_to_three_dict(self.read_key, sender_id, stream_id, sym_get_key(r_pars.bulk_cipher_algorithm, EnumTrafor().to_enum(r_pars.enc_key_length), r_pars.bulk_cipher_algorithm_option, ser_write_ky))    
            G().add_to_three_dict(self.read_mac_key, sender_id, stream_id, MACKey(r_pars.mac_algorithm, r_pars.mac_key_length, ser_write_mac_ky))
            G().add_to_three_dict(self.read_iv, sender_id, stream_id, ser_iv)            
    ...
    def _init_project_parameters(self):
        ...
        # ciphersuite
        self.available_ciphersuites = [TLSCipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, TLSCipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, TLSCipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA256, \
        ...
        # certificates
        self.certificate = {}  # generated with negotiated Algorithms per Partner
        self.lst_root_certs_to_verify_mine = {}  # all certificates necessary to validate the current ECUs certificate
        
        # public and private keys for the certificate encryption
        self.server_cert_pub_keys = {}
        self.server_cert_priv_keys = {}        
        self.client_cert_pub_keys = {}
        self.client_cert_priv_keys = {}
...
class TLSRecordLayerPreparation(object):
    ...
    def receiver_data(self, message, sender_id, stream_id):
        ...
        protocol_version = message.prot_version
        length = message.length
        tls_cipher = message.fragment      
        
        # clear message from TLSCiphertext
        clear_message = yield self.sim_env.process(self._uncipher_message(tls_cipher, sender_id, stream_id))
    ...
    def sender_data(self, message, content_type, protocol_version, sequence_nr, sender_id, desination_id, stream_id):
        ...
        # plain text
        tls_plaintext = TLSPlaintext(content_type, protocol_version, len(message), message)

        # compress text
        tls_compressed, size_compressed, compression_time = self._create_compressed_message_send(message, tls_plaintext, content_type, protocol_version)
        yield self.sim_env.timeout(compression_time)        
        
        # record layer protection - create fragment
        fragment, length = yield self.sim_env.process(self._cipher_fragment(tls_compressed, size_compressed, self._cur_sec_params.cipher_type, \
                                                                            sequence_nr, sender_id, desination_id, stream_id))
        
        # wrap in cipher object
        tls_cipher = TLSCiphertext(content_type, protocol_version, length, fragment)
        tls_cipher.stream_id = stream_id
        
        # prepare message
        cipher_message = SegData(tls_cipher, length + 5)
    ...
    def _extract_block_cipher_info(self, target_id, stream_id, content):
        ...
        iv = self._record.write_iv[target_id][stream_id]
    ...
    def _uncipher_message(self, tls_ciphertext, sender_id, stream_id):
        ''' this method reverts the encryption process of the record layer and is
            executed after reception of the message. The message will be decompressed
            and decrypted using the negotiated mechanisms. Then it is returned.
            
            Input:  tls_cipher    TLSCiphertext        encrypted cipher text that was received
                    sender_id     string               id of the ECU sending the message
                    stream_id     integer              id of the stream that was received
            Output: result        SegData              the clear message after decryption
        '''
        
        try:
            # decrypt (read key)
            encrypted_message = tls_ciphertext.fragment
            tls_compressed_msg = sym_decrypt(encrypted_message, self._record.read_key[sender_id][stream_id], self._record.sim_env.now)
            
            # decryption failed
            if tls_compressed_msg == None:  raise Exception()
            
            # decompress
            tls_plaintext, decompression_time, size_compressed = self._decompress_receive(tls_compressed_msg)            
            yield self.sim_env.timeout(decompression_time)
            
            # verify MAC
            yield self.sim_env.timeout(self._create_mac_verification_receive(size_compressed))
            
            # create verification mac          
            verification_mac = encryption_tools.mac(tls_compressed_msg, self._record.read_mac_key[sender_id][stream_id])
            
            # verify MAC
            if not encryption_tools.same_mac(tls_ciphertext.mac, verification_mac): return None
            
            # return plain text
            result = tls_plaintext.fragment
            
        except:
            # message not encrypted   
            result = self._uncipher_not_encrypted(tls_ciphertext)
            
        # depending on (object or SegData)
        if isinstance(result, SegData): return result.get()
        else: return result
...
class TLSHandshake(object):
    ...
    def __init__(self, env, tls_record, ecu_id, monitor_lst=[], jitter=1):
        ...
        # cached messages
        self._client_hello_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._server_hello_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._server_certificate_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._server_key_exchange_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._certificate_request_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._server_hello_done_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        
        self._client_certificate_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._client_key_exchange_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._certificate_verify_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._client_finish_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
        self._server_finish_msg = {}  # save it: receiverECU, associated Session ID (mapped from Stream ID)
    ...
    def send_client_hello(self, sender_id, stream_id, receiver_id):
        ...
        if "CLIENT_HALLO" in TLSCommModule.RANDOM_BYTES.keys():
            random_bytes = TLSCommModule.RANDOM_BYTES["CLIENT_HALLO"]
        else:
            random_bytes = os.urandom(28)
            TLSCommModule.RANDOM_BYTES["CLIENT_HALLO"] = random_bytes
        ...
        save_message = ObjectMap().request(clear_message)
        G().add_to_three_dict(self._client_hello_msg, receiver_id, stream_id, save_message)
    ...
    def _cache_server_hello(self, client_id, stream_id, clear_message, message_size, cipher_suite, compression_method):
        ...
        G().add_to_three_dict(self._server_hello_msg, client_id, stream_id, clear_message)
    ...
    def _certificate_from_cipher_suite(self, suite, ca_length):
        ...
        # pull certificate
        ecu_certificate, list_root_certificates, private_key = manager.generate_valid_ecu_cert(self.ecu_id, ca, 0, float('inf'), version=1.0)
        
        # result
        return ecu_certificate, list_root_certificates, private_key
    ...
    def _check_certificate_verify_caches(self, sender_id, stream_id, clear_message):
        ...
        '''
        # cached messages
        client_hello = self._client_hello_msg[sender_id][stream_id].get()
        server_hello = self._server_hello_msg[sender_id][stream_id].get()
        server_cert = self._server_certificate_msg[sender_id][stream_id].get()
        cert_request = self._certificate_request_msg[sender_id][stream_id].get()
        serv_hello_done = self._server_hello_done_msg[sender_id][stream_id].get()
        client_cert = self._client_certificate_msg[sender_id][stream_id].get()[3]
        client_keyex = self._client_key_exchange_msg[sender_id][stream_id].get()[3][0]
        
        same = True
        # received messages
        try:
            if clear_message[0] != client_hello: same = False
            if clear_message[1] != server_hello: same = False
            if clear_message[2] != server_cert: same = False
            if clear_message[3] != cert_request: same = False
            if clear_message[4] != serv_hello_done: same = False
            if clear_message[5] != client_cert: same = False
            if clear_message[6] != client_keyex: same = False
        except:
            same = False
    ...
    def _cipher_suite_supported(self, suites_incoming, client_id, stream_id):
        ...
        for suite in suites_incoming:
            if suite in self._record.available_ciphersuites:
                return True    
    ...
    def _client_finished_verification_data(self, sender_id, stream_id):
        ...
        client_hello = self._client_hello_msg[sender_id][stream_id].get()
        server_hello = self._server_hello_msg[sender_id][stream_id].get()[3]
        server_cert = self._server_certificate_msg[sender_id][stream_id].get()[3]
        cert_request = self._certificate_request_msg[sender_id][stream_id].get()[3]
        serv_hello_done = self._server_hello_done_msg[sender_id][stream_id].get()[3]
        client_cert = self._client_certificate_msg[sender_id][stream_id].get()
        client_keyex = self._client_key_exchange_msg[sender_id][stream_id].get()
        finished_msg = self._client_finish_msg[sender_id][stream_id].get()     
        msg_clear = [client_hello, server_hello, server_cert, cert_request, serv_hello_done, client_cert, client_keyex, finished_msg]
    ...
    def _client_hello_received_save_state(self, message, client_id):
        ...
        # cache
        save_message = ObjectMap().request(message[3])
        G().add_to_three_dict(self._client_hello_msg, client_id, stream_id, save_message)
    ...
    def _client_key_exchange_set_pending_configuration(self, server_id, stream_id, pre_master_secret, uuu_id):
        ...
        client_random = self._client_hello_msg[server_id][stream_id].get()[2]
        server_random = self._server_hello_msg[server_id][stream_id].get()[3][1]
        master_secret = self._generate_master_secret(self._record.negotiated_cipher_suite[server_id][stream_id], pre_master_secret, client_random, server_random)
        ...
        # information
        cipher_suite = self._record.negotiated_cipher_suite[server_id][stream_id]
        compression_method = self._record.negotiated_compression_method[server_id][stream_id]
        prf = self._dummy_prf
    ...
    def _create_client_finished_clear_message(self, sender_id, stream_id):
        ...
        clear_message = [client_hello, server_hello, server_cert, cert_request, serv_hello_done, client_cert, client_keyex]
    ...
    def _create_server_finished_clear_message(self, sender_id, stream_id):
        ...
        # create message
        clear_message = [client_hello, server_hello, server_cert, cert_request, serv_hello_done, client_cert, client_keyex, finished_msg]
    ...
    def _generate_certificate_verify_clear(self, server_id, stream_id, uid):
        ...
        # gather messages
        client_hello = self._client_hello_msg[server_id][stream_id].get()
        server_hello = self._server_hello_msg[server_id][stream_id].get()[3]
        server_cert = self._server_certificate_msg[server_id][stream_id].get()[3]
#         server_keyex = self._server_key_exchange_msg[server_id]
        cert_request = self._certificate_request_msg[server_id][stream_id].get()[3]
        serv_hello_done = self._server_hello_done_msg[server_id][stream_id].get()[3]
        client_cert = self._client_certificate_msg[server_id][stream_id].get()
        client_keyex = self._client_key_exchange_msg[server_id][stream_id].get()
        
        # concatenate
        clear_message = [client_hello, server_hello, server_cert, cert_request, serv_hello_done, client_cert, client_keyex]         
        
        # public key
        public_key = server_cert[0].pub_key_user
    ...
    def _generate_master_certificate_verify(self, sender_id, stream_id):
        ...
        client_random = self._client_hello_msg[sender_id][stream_id].get()[2]
        server_random = self._server_hello_msg[sender_id][stream_id].get()[1]
    ...
    def _handle_certificate_request(self, sender_id, clear_message):
        ...
        # cache message
        save_message = ObjectMap().request(clear_message)
        G().add_to_three_dict(self._certificate_request_msg, sender_id, stream_id, save_message)
...
#         print("Verify certificate %s" % stream_id)
    ...
    def _handle_client_certificate(self, sender_id, clear_message):
        ...
        stream_id = clear_message[3][-1]
        save_message = ObjectMap().request(clear_message)
        G().add_to_three_dict(self._client_certificate_msg, sender_id, stream_id, save_message)
        ...
        root_certificate_list = clear_message[3][1]
    ...
    def _handle_client_hello(self, message, client_id):
        ...
        if not self._cipher_suite_supported(message[3][4], client_id, stream_id):            
    ...
    def _handle_client_keyexchange(self, sender_id, exchange_message):
        ...
        save_message = ObjectMap().request(exchange_message)
        G().add_to_three_dict(self._client_key_exchange_msg, sender_id, stream_id, save_message)
...
#                 print("%s: First finished %s" % (TLSCommModule.CNT, stream_id))
        ...
        # delete done streams
        self._record.negotiated_cipher_suite[sender_id].pop(stream_id, None)
        self._record.negotiated_compression_method[sender_id].pop(stream_id, None)
        self._record.certificate[sender_id].pop(stream_id, None)
        self._record.lst_root_certs_to_verify_mine[sender_id].pop(stream_id, None)
        self._client_hello_msg[sender_id].pop(stream_id, None)     
        self._client_finish_msg[sender_id].pop(stream_id, None)     
        self.pre_master_secret[sender_id].pop(stream_id, None)     
        self._server_hello_msg[sender_id].pop(stream_id, None)
        self._server_certificate_msg[sender_id].pop(stream_id, None)
        self._certificate_request_msg[sender_id].pop(stream_id, None)
        self._server_hello_done_msg[sender_id].pop(stream_id, None)                
        self._client_certificate_msg[sender_id].pop(stream_id, None)
        self._client_key_exchange_msg[sender_id].pop(stream_id, None)        
...
#         ObjectMap().remove_unreferenced()
    ...
    def _handle_server_certificate(self, sender_id, message):
        ...
        # cache message
        save_message = ObjectMap().request(message)
        G().add_to_three_dict(self._server_certificate_msg, sender_id, stream_id, save_message)
    ...
    def _handle_server_hello(self, sender_id, message):
        ...
        # cache message
        save_message = ObjectMap().request(message)
        G().add_to_three_dict(self._server_hello_msg, sender_id, stream_id, save_message)
        
        # initialize empty suite
        G().add_to_three_dict(self._record.negotiated_cipher_suite, sender_id, stream_id, {})
        G().add_to_three_dict(self._record.negotiated_compression_method, sender_id, stream_id, {})                        
    ...
    def _handle_server_hello_done(self, sender_id, message):
        ...
        cert, lst_root_certs, str_id = self._server_certificate_msg[sender_id][stream_id].get()[3]  # @UnusedVariable
        ...
        if sender_id in self._certificate_request_msg:
            if stream_id in self._certificate_request_msg[sender_id]:
    ...
    def _handle_server_key_exchange(self, sender_id, message):
        ...
        # cache
        save_message = ObjectMap().request(message)
        G().add_to_three_dict(self._server_key_exchange_msg, sender_id, stream_id, save_message)
    ...
    def _public_key_from_server_message(self, server_id, stream_id):
        ...
        '''
        # message
        certificate_message = self._server_certificate_msg[server_id][stream_id].get()
        
        # server certificate
        server_certificate = certificate_message[3][0]
    ...
    def _send_certificate_request(self, client_id, stream_id):
        ...
        # cache message
        save_message = ObjectMap().request(clear_message)
        G().add_to_three_dict(self._certificate_request_msg, client_id, stream_id, save_message)
    ...
    def _send_client_certificate(self, server_id, stream_id):
        ...
        ca_length = 1
        suite = self._record.negotiated_cipher_suite[server_id][stream_id]
        certificate, root_certificate_list, priv_key = self._certificate_from_cipher_suite(suite, ca_length)  # @UnusedVariable
        
        # cache certificate                 
        G().add_to_three_dict(self._record.certificate, server_id, stream_id, certificate)
        G().add_to_three_dict(self._record.lst_root_certs_to_verify_mine, server_id, stream_id, root_certificate_list)
        ...
        # cache 
        save_message = ObjectMap().request(clear_message)
        G().add_to_three_dict(self._client_certificate_msg, server_id, stream_id, save_message)
    ...
    def _send_client_keyexchange(self, server_id, stream_id):
        ...
        # cache message
        save_message = ObjectMap().request(encrypted_msg)
        G().add_to_three_dict(self._client_key_exchange_msg, server_id, stream_id, save_message)
    ...
    def _send_server_certificate(self, client_id, stream_id):
        ...
        # cache
        G().add_to_three_dict(self._record.certificate, client_id, stream_id, server_certificate)
        G().add_to_three_dict(self._record.lst_root_certs_to_verify_mine, client_id, stream_id, server_root_certificates)
        
        # client certificate
        certificate = self._record.certificate[client_id][stream_id]
        root_certificate_list = self._record.lst_root_certs_to_verify_mine[client_id][stream_id]
        
        # cache
        G().add_to_three_dict(self._record.server_cert_priv_keys, client_id, stream_id, private_key)
        G().add_to_three_dict(self._record.server_cert_pub_keys, client_id, stream_id, certificate.pub_key_user)
    ...
    def _send_server_hello(self, client_hello, client_id, stream_id):
        ...
        cipher_suite = client_hello[3][4][0]  # select first ciphersuite
    ...
    def _send_server_hello_done(self, client_id, stream_id):
        ...
        # send message
        save_message = ObjectMap().request(clear_message)
        G().add_to_three_dict(self._server_hello_done_msg, client_id, stream_id, save_message)
    ...
    def _send_server_key_exchange(self, client_id, stream_id):
        ...
        save_message = ObjectMap().request(msg_clear)
        G().add_to_three_dict(self._server_key_exchange_msg, client_id, stream_id, save_message)
    ...
    def _sendable_certificate_verify(self, cipher_size, encrypted_msg, stream_id, server_id, clear_message, uuu_id):
        ...
        # cache message
        save_message = ObjectMap().request(clear_message)
        G().add_to_three_dict(self._certificate_verify_msg, server_id, stream_id, save_message)
    ...
    def _sendable_client_certificate(self, certificate, root_certificate_list, stream_id, server_id):
        ...
        message_size = self._record.TLSH_CERT_SEND_SIZE + (len(root_certificate_list) - 1) * self._record.TLSH_CERT_SEND_SIZE
    ...
    def _sendable_client_finished_message(self, verification_data, stream_id, sender_id, uuuid):
        ...
        # cache message
        save_data = ObjectMap().request(verification_data)
        G().add_to_three_dict(self._client_finish_msg, sender_id, stream_id, save_data)
    ...
    def _sendable_server_certificate(self, certificate, root_certificate_list, stream_id, client_id):
        ...
        # cache
        save_message = ObjectMap().request(clear_message)
        G().add_to_three_dict(self._server_certificate_msg, client_id, stream_id, save_message)
    ...
    def _sendable_server_finished_message(self, verification_data, stream_id, sender_id, uuuid):
        ...
        # cache 
        save_data = ObjectMap().request(verification_data)
        G().add_to_three_dict(self._server_finish_msg, sender_id, stream_id, save_data)
    ...
    def _server_finished_verification_data(self, sender_id, stream_id):
        ...
        client_cert = self._client_certificate_msg[sender_id][stream_id].get()[3]
        client_keyex = self._client_key_exchange_msg[sender_id][stream_id].get()[3][0]            
        msg_clear = [client_hello, server_hello, server_cert, cert_request, serv_hello_done, client_cert, client_keyex]
...
#         logging.info(log_str)
    ...
    def _set_client_finished_message_state(self, sender_id, stream_id, message):
        ...
        # set state
        save_message = ObjectMap().request(message)
        G().add_to_three_dict(self._server_finish_msg, sender_id, stream_id, save_message)
...
#         logging.info(log_str)
    ...
    def _set_server_finished_message_state(self, sender_id, stream_id, message):
        ...
        # set state
        save_message = ObjectMap().request(message)
        G().add_to_three_dict(self._client_finish_msg, sender_id, stream_id, save_message)
    ...
    def _set_server_hello_done_state(self, sender_id, message):
        ...
        # cache message
        save_message = ObjectMap().request(message)
        G().add_to_three_dict(self._server_hello_done_msg, sender_id, stream_id, save_message)
        ...
        # save negotiated suite
        negotiated_suite = self._server_hello_msg[sender_id][stream_id].get()[3][3]
        negotiated_compression = self._server_hello_msg[sender_id][stream_id].get()[3][4]
        G().add_to_three_dict(self._record.negotiated_cipher_suite, sender_id, stream_id, negotiated_suite)
        G().add_to_three_dict(self._record.negotiated_compression_method, sender_id, stream_id, negotiated_compression)
...
class ObjectMap(Singleton):
    ...
    def request_sec_params(self, sec_params):
        ''' once a sec parameters is requested the object 
            ...
            Output:   output             dict/list         the requested list or dictionary
        '''
        for entry in self._sec_params_map:
            if entry.equals(sec_params):
                return entry    
        self._sec_params_map.append(sec_params)
        return sec_params
