...
def main(argv):
    ...
    file = open(path + 'systemStartOverReceivers.txt', 'w')
    file.write("receivers\tavg\tmin\tmax\n")
...
#     for e in range(0, 1):
    for e in range(0, 11):
        print("receiverCoeff: " + str(e/10))
        for r in range(0, 21):
            coeff = e / 10

            filename = "varyReceivers-" + str(coeff) + "-run-" + str(r) + ".csv"
            ...
            '''find number of receivers per stream'''
            numberOfReceivers = dict()
            for start in StreamAuthStart:
                for end in StreamAuthEnd:
                    if(start[1] == end[1]):
                        if not start[1] in numberOfReceivers:
                            numberOfReceivers[start[1]] = 1
                        else:
                            numberOfReceivers[start[1]] = numberOfReceivers[start[1]] + 1

#             for x in numberOfReceivers:
#                 print(x + ":" + str(numberOfReceivers[x]))

            '''find last grant message per stream'''
            streamSetupTime = dict()
            for start in numberOfReceivers:
                for end in StreamAuthEnd:
                    if(start == end[1]):
                        if not start in streamSetupTime:
                            streamSetupTime[start] = end[0]
#                             print(str(end[0]))
                        elif (end[0] > streamSetupTime[start]):
                            streamSetupTime[start] = end[0]
#                             print(str(end[0]))

#             for x in StreamAuthStart:
#                 streamSetupTime[x[1]] = float(streamSetupTime[x[1]]) - float(x[0])

#             for x in streamSetupTime:
#                 print(str(numberOfReceivers[x])+":"+str(streamSetupTime[x]))

            for x in streamSetupTime:
                collectedTimes[numberOfReceivers[x]].append(streamSetupTime[x])
#                 print("numberOfReceivers[x]="+str(numberOfReceivers[x]))
